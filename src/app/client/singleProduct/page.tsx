...
export default function Page() {
  ...
  // };
  const clientId: any = localStorage.getItem("userId");

  const handlePost = async () => {
    const time: any = new Date();
    const newData = new FormData();
    newData.append("count", data?.count);
    newData.append("created_at", time);
    newData.append("client_id", clientId);
    newData.append("product_id", id);

    const res = await instance.post("post/order", newData);
      ...
      <div className="teacher_box  overflow-hidden  dark:bg-[#1F2937] mt-[40px] bg-white rounded-[20px] w-[100%] h-auto ">
        ...
        <div className=" text-right  py-[35px] ">
          <button
            ...
            className="text-[14px] md:text-[18px] font-bold text-[#4D44B5] border-[1px] border-[#4D44B5]  bg-transparent md:py-2 py-[10px] px-[28px]  rounded-[40px]   "
          >
            Savatga qo’shish
          </button>
          ...
          <button
            onClick={handlePost}
            className="text-[14px] md:text-[18px] font-bold text-white  mx-[25px]  bg-[#4CBC9A] md:py-2 py-[10px] px-[28px]  rounded-[40px]"
          >
            Sotib olish
          </button>
        </div>
