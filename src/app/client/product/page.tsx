        ...
        <div className=" gird  grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-4  ">
          {data.length ? (
            data?.map((element: any) => (
              <Link
                href="/client/singleProduct"
                className="teacher_card relative dark:bg-[#1F2937]    min-h-[352px] text-center rounded-[20px] bg-[#C1BBEB]  pt-[15px] pb-[53px] px-[15px] "
                onClick={() => {
                  window.localStorage.setItem("id", element?.id);
                }}
              >
                <div className="rounded-[15px] bg-white   w-auto h-[255px] overflow-hidden ">
                  <Image
